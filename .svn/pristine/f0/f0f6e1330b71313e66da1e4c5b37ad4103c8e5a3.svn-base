 ...
 * 
 */
public class ShouYeMainRunFragment extends BaseFragment implements
	...
	protected View initView(LayoutInflater inflater) {
		View view = inflater.inflate(R.layout.fragment_shouye_main_run, null,
				false);
		lv_main_run = (XListView) view.findViewById(R.id.lv_main_run);
	...
	public void init() {
		...
		vhu.ListProducts(getActivity(), "1", "10", "1", false);
		vhu.setObjectList(this);
	}

	public void init(String eachnum) {
		VolleyHttpUtil vhu = VolleyHttpUtil.getInstance(getActivity());
		vhu.ListProducts(getActivity(), "1", eachnum, "1", false);
		vhu.setObjectList(this);
	}
	...
	public void ReturnVolleyString(String response, String type) {
		...
		try {
			...
			if (code == 200) {
				...
				if (array.length() == 0) {
					Toast.makeText(getActivity(), R.string.nodatas, Toast.LENGTH_SHORT)
							.show();
					...
					adapter = new ShouYeMainListAdapter(getActivity(), array,
							times, 0);
					lv_main_run.setPullRefreshEnable(true);
					lv_main_run.setPullLoadEnable(true);
					lv_main_run.setXListViewListener(this);
					lv_main_run.setAdapter(adapter);
					if (index == 0) {
	...
	public void onLoadMore() {
		if (bolg) {
			lv_main_run.setRefreshTime(":" + DateTimeUtil.getNowTime());
			bolg = false;
		} else {
			lv_main_run.setRefreshTime(":" + date);
		}
	...
	@Override
	public void onLoad() {
		lv_main_run.stopRefresh();
		lv_main_run.stopLoadMore();
		adapter.notifyDataSetChanged();
		// lv_main_run.setSelection((pullpage * 10) - 1);
	}
